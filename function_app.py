...
@app.function_name(name="QueueConnectivityTest")
@app.route(route="queue-test", methods=["GET"], auth_level=func.AuthLevel.FUNCTION)
def queue_connectivity_test(req: func.HttpRequest) -> func.HttpResponse:
    try:
        queue_name = os.environ.get("QUESTION_QUEUE_NAME", "iviva-question-generation")
        conn_str = os.environ.get("AzureWebJobsStorage")

        if not conn_str:
            return func.HttpResponse(
                "AzureWebJobsStorage is missing in app settings.",
                status_code=500,
            )
        ...
        # peek does not change DequeueCount
        peeked = qc.peek_messages(max_messages=1)

        out = {
            "ok": True,
            "queue_name": queue_name,
            "approximate_message_count": getattr(props, "approximate_message_count", None),
            "peek_count": len(peeked) if peeked else 0,
            "peek_sample": (peeked[0].content if peeked else None),
        }
        return func.HttpResponse(
        ...
        logging.exception("QueueConnectivityTest failed")
        return func.HttpResponse(
